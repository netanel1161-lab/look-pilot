(() => {
    ...
    async function init() {
        await Promise.all([loadFromDefault(), loadUsersFromDefault()]);
        bindEvents();
        renderAll();
    }
    ...
    async function loadUsersFromDefault() {
        try {
            const res = await fetch('../data/users.json');
            if (!res.ok) throw new Error('users.json לא נמצא');
            const data = await res.json();
            state.users = Array.isArray(data) ? data : [];
            setUsersStatus('users.json נטען לעריכה.');
        } catch (err) {
            state.users = [];
            setUsersStatus('לא נמצא users.json, נתחיל עם רשימה ריקה.');
        }
    }
    ...
    function bindEvents() {
        ...
        if (els.addUser) {
            els.addUser.addEventListener('click', () => {
                state.users.push({ name: '', phone: '' });
                renderUsers();
                setUsersStatus('שינויים לא נשמרו.');
            });
        }

        if (els.usersList) {
            els.usersList.addEventListener('input', handleUserInput);
            els.usersList.addEventListener('click', handleUserRemove);
        }

        if (els.saveUsers) {
            els.saveUsers.addEventListener('click', saveUsersFile);
        }
    ...
    function handleUserInput(event) {
        ...
        if (field === 'phone') {
            ...
            state.users[index][field] = clean;
        } else {
            state.users[index][field] = event.target.value;
        }
        renderUsers();
        setUsersStatus('שינויים לא נשמרו.');
    }
    ...
    function handleUserRemove(event) {
        const btn = event.target.closest('[data-remove-user]');
        if (!btn) return;
    ...
    async function saveUsersFile() {
        ...
        if (window.showSaveFilePicker) {
            try {
                ...
                await writable.write(blob);
                await writable.close();
                setUsersStatus('users.json נשמר.');
                return;
            } catch (err) {
                setUsersStatus('שמירת users.json בוטלה או נכשלה.');
            }
        }
        ...
        link.click();
        URL.revokeObjectURL(link.href);
        setUsersStatus('הקובץ הורד כ-users.json');
    }
    ...
    function renderUsers() {
        ...
        els.usersList.innerHTML = state.users.map((user, index) => `
            <div class="list-row user-row" data-index="${index}">
                <div class="grid two">
                    <div>
                        <label>שם</label>
                ...
                <div class="controls">
                    <button type="button" class="secondary" data-remove-user>מחק משתמש</button>
                </div>
            </div>
        `).join('') || '<div class="muted">אין משתמשים, הוסף אחד.</div>';
    }
